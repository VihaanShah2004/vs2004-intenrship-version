...
class PopupManager {
    ...
    updateWebsiteInfo() {
        if (this.currentWebsite) {
            ...
            if (this.currentWebsite.recommendedCardId) {
                const brandCard = this.creditCards.find(card => card.id === this.currentWebsite.recommendedCardId);
                if (brandCard) {
                    brandRecommendation = `<br><strong>🎯 Perfect Match:</strong> ${brandCard.name}`;
                }
            }
            ...
            websiteInfo.innerHTML = `
                <div style="background: rgba(255,255,255,0.1); padding: 10px; border-radius: 8px; margin-bottom: 15px; font-size: 12px;">
                    ...
                    <strong>Confidence:</strong> ${Math.round(this.currentWebsite.confidence * 100)}%${brandRecommendation}
                </div>
            `;
